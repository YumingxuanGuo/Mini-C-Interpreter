...
int *pc,              // program counter
    *bp,              // base pointer
    *sp,              // stack pointer
    ax,               // general purpose registers (accumulator)
    cycle;

int token_val;        // value of current token (mainly for numbers)
int *current_id,      // current parsed ID
    *symbols;         // symbol table to replace struct
...
// 5: local var 1
// 6: local var 2
int index_of_bp;  // index of base point on stack
...
void next() {
  ...
  while (token = *src) {
    ...
    if (token == '\n') {
      ...
      // parse number of three kinds: dec(123) hex(0x123) oct(017)
      token_val = token - '0';
      if (token_val > 0) {
        // dec, starts with [1-9]
        while (*src >= '0' && *src <= '9') {
      ...
      // currently, the only supported escape character is '\n'
      ...
      src++;
      // if it is a single cahracter, return Num token
      if (token == '"') {
...
void expression(int level) {
  ...
  if (token == Num) {
    ...
    *++text = token_val;
    ...
    match('"');
    // store the rest of the strings
    while (token == '"') {
      match('"');
    }

    // append the end of string character '\0'
    // all the data are default to 0, so just move data one position forward
    data = (char *)(((int)data + sizeof(int)) & (-sizeof(int)));
    expr_type = PTR;
  } else if (token == Sizeof) {
    // size of is actually an unary operator
...
void function_parameter() {
  ...
  while (token != ')') {
    ...
    // parameter name
    if (token != Id) {
      printf("%lld: ba parameter declaration\n", line);
      exit(-1);
    }
...
void function_declaration() {
  ...
  match('{');
  function_body();
  // '}' is left in the outer while loop
...
int eval() {
  ...
  while (1) {
    ...
    // error catching
      ...
      printf("unkonwn instruction: %lld\n", op);
      return -1;
    }
  }
  return 0;
